...
const TRANSLATIONS = {
	fr: { menu: 'Menu', trending: 'Trending', favorites: 'Favoris', reviews: 'Avis', account: 'Mon Compte', comingSoon: 'Bientôt' },
	en: { menu: 'Menu', trending: 'Trending', favorites: 'Favorites', reviews: 'Reviews', account: 'My Account', comingSoon: 'Soon' },
	es: { menu: 'Menú', trending: 'Trending', favorites: 'Favoritos', reviews: 'Reseñas', account: 'Mi Cuenta', comingSoon: 'Pronto' },
};
...
const Sidebar = () => {
	...
	return (
		<div className={`fixed left-0 top-0 h-full bg-gray-900 border-r border-gray-800 z-50 flex flex-col transition-all duration-300 ${collapsed ? 'w-16' : 'w-56'}`}>
			<div className='flex items-center justify-between p-4 border-b border-gray-800'>
				...
				<button onClick={() => setCollapsed(!collapsed)} className='text-gray-400 hover:text-white transition-colors ml-auto text-lg'>
					{collapsed ? '›' : '‹'}
				</button>
			</div>
						...
						{item.comingSoon ? (
							...
							<Link
								...
								className={`flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 ${collapsed ? 'justify-center' : ''} ${
									isActive(item.path) ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-gray-400 hover:bg-gray-800 hover:text-white'
								}`}>
								<span className='text-xl flex-shrink-0'>{item.icon}</span>
			...
			<div className='p-2 border-t border-gray-800'>
				<Link
					to='/account'
					className={`flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 ${collapsed ? 'justify-center' : ''} ${
						isActive('/account') ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'
					}`}>
