...
const getWeekKey = () => {
	...
	return `trending_v3_${now.getFullYear()}_w${week}`;
};

const TABS = [
	{ key: 'movies', label: 'Films', icon: '🎬', color: 'blue', activeClass: 'bg-blue-600 shadow-blue-900/60' },
	{ key: 'series', label: 'Séries', icon: '📺', color: 'purple', activeClass: 'bg-purple-600 shadow-purple-900/60' },
	{ key: 'anime', label: 'Anime', icon: '⛩️', color: 'pink', activeClass: 'bg-pink-600 shadow-pink-900/60' },
];
...
const TrendingCard = ({ item, onClick }) => {
	...
	const rankColor = item.rank === 1 ? 'text-yellow-400' : item.rank === 2 ? 'text-gray-300' : item.rank === 3 ? 'text-amber-600' : 'text-gray-600';
	...
	return (
		<div
			...
			<div className={`absolute inset-0 transition-opacity duration-300 ${
				backdropUrl
					? 'bg-gradient-to-r from-gray-950/98 via-gray-900/90 to-gray-900/50'
					: 'bg-gray-800'
			}`} />
			...
			<div className='relative flex items-center gap-5 px-6 py-5'>
				{/* Rank */}
				<div className={`flex-shrink-0 w-14 text-center text-6xl font-black leading-none ${rankColor} drop-shadow-lg`}>
					{item.rank}
				</div>
				...
				{posterUrl && (
					<div className='flex-shrink-0'>
						<img
							src={posterUrl}
							alt={item.title}
							className='w-16 h-24 object-cover rounded-xl shadow-xl group-hover:scale-105 transition-transform duration-300'
				...
				<div className='flex-1 min-w-0'>
					<h3 className={`font-bold text-xl truncate mb-1 transition-colors duration-200 ${hovered ? 'text-blue-300' : 'text-white'}`}>
						{item.title}
					</h3>
				...
				{/* Right stats */}
				<div className='flex-shrink-0 text-right hidden lg:block min-w-24'>
					<div className='text-2xl font-black text-white'>{voteDisplay}</div>
					<div className='text-blue-400 text-xs'>votes</div>
					<div className='mt-2 text-xl font-bold text-white'>{item.vote_average}</div>
...
const Trending = () => {
	...
	const loadTrending = async () => {
		...
		Object.keys(localStorage).forEach(k => {
			if (k.startsWith('trending_v3_') && k !== weekKey) localStorage.removeItem(k);
		});
		...
		<div className='min-h-screen bg-black'>
			...
			<div className='relative flex flex-col items-center justify-center py-24 px-6 overflow-hidden'>
				<div className='absolute inset-0'>
					<div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-blue-600/15 rounded-full blur-3xl' />
					{activeTab === 'anime' && (
						<div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-pink-600/15 rounded-full blur-3xl' />
					)}
				</div>
				<div className='relative text-center'>
					<h1 className='text-8xl font-black leading-none tracking-tighter mb-4'
						style={{
							color: activeTab === 'anime' ? '#f472b6' : activeTab === 'series' ? '#a78bfa' : '#60a5fa',
							textShadow: `0 0 80px ${activeTab === 'anime' ? 'rgba(244,114,182,0.4)' : activeTab === 'series' ? 'rgba(167,139,250,0.4)' : 'rgba(96,165,250,0.4)'}`,
						}}>
						TOP 10
					</h1>
					<p className='text-3xl font-black text-white uppercase tracking-widest mb-3'>
						{activeTab === 'movies' ? 'Films les plus populaires' : activeTab === 'series' ? 'Séries les plus populaires' : 'Animes les plus populaires'}
					</p>
					<p className='text-gray-500 text-sm mb-10'>Sur les 7 derniers jours • Mis à jour chaque semaine</p>
