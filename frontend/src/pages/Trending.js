...
const GenreTag = ({ name, theme }) => (
	<span className={`text-xs px-2.5 py-1 rounded-full ${theme === 'dark' ? 'bg-white/10 text-gray-300 border-white/10' : 'bg-gray-200 text-gray-700 border-gray-300'} border`}>
		{name}
	</span>
);

const TrendingCard = ({ item, onClick, theme }) => {
	...
	return (
		<div
			...
			className={`relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 border ${theme === 'dark' ? 'border-white/5 hover:border-blue-500/40 hover:shadow-blue-900/20' : 'border-gray-200 hover:border-blue-300 hover:shadow-blue-200/50'} hover:shadow-2xl group`}
			...
			<div className={`absolute inset-0 transition-opacity duration-300 ${
				backdropUrl
					? `${theme === 'dark' ? 'bg-gradient-to-r from-gray-950/98 via-gray-900/88 to-gray-900/40' : 'bg-gradient-to-r from-white/95 via-white/85 to-white/40'}`
			...
			<div className='relative flex items-center gap-6 px-6 py-5'>
				...
				<div className='flex-shrink-0 flex items-center justify-center'>
					{posterUrl ? (
						...
						<div className={`w-20 h-28 ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-200 border-gray-300'} rounded-xl flex items-center justify-center border`}>
				...
				<div className='flex-1 min-w-0'>
					<h3 className={`font-bold text-xl truncate mb-1.5 transition-colors duration-200 ${hovered ? 'text-blue-300' : `${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}`}>
						{item.title}
					</h3>
					<div className='flex items-center gap-3 mb-3'>
						{year && <span className={`text-sm ${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'}`}>{year}</span>}
						<span className='text-yellow-400 text-sm font-bold'>★ {item.vote_average}/10</span>
					</div>
					<div className='flex flex-wrap gap-1.5'>
						{genres.map(g => <GenreTag key={g} name={g} theme={theme} />)}
					</div>
				</div>

				<div className='flex-shrink-0 text-right hidden lg:block min-w-28'>
					<div className={`text-2xl font-black ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{voteDisplay}</div>
					<div className='text-blue-400 text-xs mb-3'>votes</div>
					<div className={`text-xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{item.vote_average}</div>
					<div className='text-blue-400 text-xs'>note moy.</div>
				</div>

				<div className={`flex-shrink-0 text-2xl transition-all duration-200 ${hovered ? 'text-blue-400 translate-x-1' : `${theme === 'dark' ? 'text-gray-700' : 'text-gray-400'}`}`}>›</div>
			</div>
		</div>
	);
};
...
const Trending = () => {
	...
	if (isLoading) return (
		<div className={`min-h-screen ${theme === 'dark' ? 'bg-black' : 'bg-gray-100'} flex items-center justify-center`}>
			<div className='text-center'>
				...
				<p className={`text-xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Chargement du Top 10...</p>
				<p className={`text-sm mt-1 ${theme === 'dark' ? 'text-gray-600' : 'text-gray-500'}`}>Données TMDB en cours de récupération</p>
			</div>
		</div>
	);
		...
		<div className={`min-h-screen ${theme === 'dark' ? 'bg-black' : 'bg-gray-100'}`}>
			<div className='relative flex flex-col items-center justify-center py-24 px-6 overflow-hidden'>
				...
				<div className='relative text-center'>
					...
					<p className={`text-3xl font-black uppercase tracking-widest mb-3 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
						{activeTab === 'movies' ? 'Films les plus populaires'
							: activeTab === 'series' ? 'Séries les plus populaires'
							: 'Animes les plus populaires'}
					</p>
					<p className={`text-sm mb-10 ${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'}`}>
						Sur les 7 derniers jours • Mis à jour chaque semaine
					</p>

					<div className={`flex gap-2 ${theme === 'dark' ? 'bg-gray-900' : 'bg-white'} p-1.5 rounded-2xl border ${theme === 'dark' ? 'border-gray-800' : 'border-gray-200'} w-fit mx-auto`}>
						{TABS.map(tab => (
							<button
								key={tab.key}
			...
			<div className={`grid grid-cols-2 md:grid-cols-4 border-t border-b ${theme === 'dark' ? 'border-gray-800/50' : 'border-gray-300'} mb-10`}>
				{[
					...
					<div key={i} className={`flex flex-col items-center justify-center py-8 border-r ${theme === 'dark' ? 'border-gray-800/50' : 'border-gray-300'} last:border-r-0`}>
						<span className='text-2xl mb-1'>{stat.icon}</span>
						<span className={`text-xs uppercase tracking-widest mb-2 ${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'}`}>{stat.label}</span>
						<span className={`text-4xl font-black ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{stat.value}</span>
					</div>
				))}
			</div>
			...
			<div className='max-w-5xl mx-auto px-6 pb-16 space-y-3'>
				{currentData.length === 0 ? (
					<div className='text-center py-20'>
						<p className='text-5xl mb-4'>🎬</p>
						<p className={`text-lg ${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'}`}>Aucun contenu disponible</p>
					</div>
				) : (
					currentData.map(item => (
						<TrendingCard key={item.id} item={item} onClick={handleClick} theme={theme} />
					))
				)}
			</div>

			<p className={`text-center text-xs pb-8 ${theme === 'dark' ? 'text-gray-800' : 'text-gray-500'}`}>
